...
import { EASE, Button } from "../../lib/ui";
import { TextField, SelectField, TextAreaField } from "../application/formFields";
...
export default function TourBookingForm() {
  ...
  if (state === "success") {
    return (
      <motion.div
        ...
        className="rounded-3xl border border-navy/10 bg-white p-8 text-center shadow-[0_20px_50px_-24px_rgba(8,8,8,0.25)]"
      >
        ...
        <h3 className="mt-5 font-display text-3xl font-semibold text-navy">Tour Request Received!</h3>
        <p className="mt-3 text-base leading-relaxed text-ink/65">
          Thank you, {requestorName || "friend"}! Our team will contact you shortly to confirm your visit to
          Blessedville Schools.
        </p>
        <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
          <button
            type="button"
            onClick={() => setState("idle")}
            className="inline-flex items-center justify-center gap-2 rounded-full border border-navy/20 px-7 py-3.5 text-sm font-semibold text-navy transition-all hover:border-navy hover:bg-navy hover:text-white"
          >
  ...
  return (
    <div className="rounded-3xl border border-navy/10 bg-white p-5 shadow-[0_20px_50px_-24px_rgba(8,8,8,0.25)] sm:p-8">
      <h3 className="font-display text-2xl font-semibold text-navy sm:text-3xl">Request a Visit</h3>
      <p className="mt-2 text-sm leading-relaxed text-ink/60">
        Tell us when you'd like to visit and we'll confirm a time with you. Visits take place during the week by
        appointment.
      </p>
